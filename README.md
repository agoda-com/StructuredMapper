...
## Performance
Building a mapper is relatively slow (see [MapperBuilderPerformanceTests.cs](/StructuredMapper.Test.Performance/MapperBuilderPerformanceTests.cs)) as each target expression must be [compiled](/StructuredMapper/PropertyMapper.cs#L70) at runtime. As an optimization, compilation is lazy, so will only occur the first time the property is actually mapped. If the property is never mapped (or, indeed, the mapper is not used at all), then there is no runtime compilation penalty. Mapper build time increases linearly for each mapped property. But there is good news.
